...
# Description: Implementation of the HashMap class with chaining
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        if self._buckets.get_at_index(index).length() != 0:
            status = self._buckets.get_at_index(index).contains(key)
            if status == None:
    ...
    def table_load(self) -> float:
        ...
        for i in range(self._capacity):
            if self._buckets.get_at_index(i).length() == 0:
                ...
                elements_count += self._buckets.get_at_index(i).length()
                buckets_count += 1
        return elements_count / buckets_count
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        new hash map, and all hash table links must be rehashed. If new_capacity is less than 1, the method does
        nothing.
        """
        if new_capacity < 1:
            return
            ...
            for i in range(old_buckets.length()):
                if old_buckets.get_at_index(i).length() == 0:
                    continue
                else:
                    for x in old_buckets.get_at_index(i):
    ...
    def get_keys(self) -> DynamicArray:
        ...
        if self._size == 0:
            ...
            for x in range(self._capacity):
                if self._buckets.get_at_index(x).length() != 0:
                    for i in self._buckets.get_at_index(x):
                        DA.append(i.key)
            return DA

def find_mode(da: DynamicArray) -> (DynamicArray, int):
    """
    Function that receives a DynamicArray (that is not guaranteed to be sorted). This function will return a tuple
    containing, in this order, a DynamicArray comprising the mode (most occurring) value/s of the array, and an integer
    that represents the highest frequency (how many times they appear). If there is more than one value with the highest
    frequency, all values at that frequency should be included in the array being returned (the order does not matter).
    ...
    for x in range(da.length()):
        current_key = da.get_at_index(x)
        if map.contains_key(current_key) == True:
            map.put(current_key, (map.get(current_key) + 1))
            if map.get(current_key) > mode_frequency:
                mode_value = DynamicArray()
                mode_value.append(current_key)
                mode_frequency = map.get(current_key)
            elif map.get(current_key) == mode_frequency:
                mode_value.append(current_key)
        else:
